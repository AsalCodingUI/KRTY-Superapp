...
type ProjectAssignmentWithScores = {
    ...
    project_sla_scores: Array<{ score_achieved: number; weight_percentage: number }>
}
...
export async function getAllEmployees(searchQuery?: string) {
    ...
    if (callerProfile?.role !== 'stakeholder') {
        return { success: false, error: "Access denied: Stakeholder role required", data: [] }
    }
    ...
    const employeesWithCounts = await Promise.all(
        (employees || []).map(async (employee) => {
            ...
            const activeProjects = (assignments as ProjectAssignmentWithStatus[] | null)?.filter(
...
export async function getEmployeeOverview(userId: string, quarter?: string) {
    ...
    const weightsConfig = configs?.find(c => c.config_key === 'kpi_weights')?.config_value as any
    const targetsConfig = configs?.find(c => c.config_key === 'kpi_targets')?.config_value as any

    const weights = weightsConfig || { sla_project: 40, review_360: 15, work_quality: 45 }
    const targets = targetsConfig || { sla_project: 90, review_360: 85, work_quality: 90 }
    ...
    if (projectAssignments && projectAssignments.length > 0) {
        const typedAssignments = projectAssignments as unknown as ProjectAssignmentWithScores[]
        const slaScores = typedAssignments.flatMap((a) => a.project_sla_scores || [])
        if (slaScores.length > 0) {
            const realAchieve = slaScores.reduce((sum, s) => sum + (s.score_achieved || 0), 0)
            const bestAchieve = slaScores.reduce((sum, s) => sum + ((s.weight_percentage || 0) * 120), 0)
    ...
    if (qualityAssignments && qualityAssignments.length > 0) {
        const typedQualityAssignments = qualityAssignments as unknown as ProjectAssignmentWithQuality[]
        const allQualityScores = typedQualityAssignments.flatMap((a) => a.project_work_quality_scores || [])
