...
export function ProjectScoringClient({ employee, assignment }: ProjectScoringClientProps) {
    const [activeTab, setActiveTab] = useState<TabType>("sla")
    const [milestones, setMilestones] = useState<Milestone[]>([
        { name: "Wireframe", weight: 30, result: "On Time", realAchieve: 3000, targetPercentage: 120 },
        { name: "High Fidelity", weight: 60, result: "On Time", realAchieve: 6000, targetPercentage: 120 },
        { name: "Handover", weight: 10, result: "On Time", realAchieve: 1000, targetPercentage: 100 },
    ])
            ...
            try {
                ...
                if (scoresResult.success && scoresResult.data) {
                    ...
                    if (scoresResult.data.slaScores.length > 0) {
                        const loadedMilestones: Milestone[] = scoresResult.data.slaScores.map((score: SLAScore) => ({
                    ...
                    const compResult = await getCompetenciesForRole(assignment.role_in_project)
                    if (compResult.success) {
                        const loadedCompetencies = compResult.data.map((comp: CompetencyLibraryItem) => {
    ...
    const handleMilestoneChange = (index: number, field: keyof Milestone, value: string | number | SLAResult) => {
    ...
    const handleSaveQuality = async () => {
        ...
        try {
            ...
            const result = await saveWorkQualityScores(assignment.id, competenciesData)
            if (result.success) {
        ...
        <div className="space-y-6">
                ...
                <div className="mt-4 flex items-start justify-between">
                    <div>
                        ...
                        {assignment.projects.description && (
                            <p className="mt-1 text-content-subtle dark:text-content-placeholder">
                                {assignment.projects.description}
                            </p>
                        )}
                    </div>
            ...
            <div className="grid grid-cols-2 gap-6">
                ...
                <Card>
                    ...
                    <dd className="mt-2 text-2xl font-semibold text-content dark:text-content">
                        {competencies.filter(c => c.isAchieved).length}/{competencies.length}
                    </dd>
                    <Badge className={`mt-2 ${getScoreColor(mapPercentageToScore(qualityPercentage))}`}>
            ...
            {/* TAB NAVIGATION */}
            <TabNavigation>
                <TabNavigationLink active={activeTab === "sla"} onClick={() => setActiveTab("sla")}>
                    SLA Calculation
                </TabNavigationLink>
                <TabNavigationLink active={activeTab === "quality"} onClick={() => setActiveTab("quality")}>
                    Work Quality
                </TabNavigationLink>
            </TabNavigation>
            ...
            {activeTab === "sla" ? (
                <div className="space-y-6">
                    ...
                    <Card className="border-0 p-0">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-4 text-sm font-medium text-content dark:text-content px-1 pb-2 border-b border-border">
                            ...
                            {milestones.map((milestone, index) => (
                                <div key={index} className="flex gap-4 items-center">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) =>
                                            handleMilestoneChange(index, "weight", Number(e.target.value))
                                        }
                                    />
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(v) => handleMilestoneChange(index, "result", v as SLAResult)}
                                    >
                            ...
                            <div className="border-t border-border pt-4">
                                <div className="flex justify-between font-semibold text-content dark:text-content">
                                    <span>Total</span>
                                    <span>{milestones.reduce((sum, m) => sum + m.weight, 0)}%</span>
                                </div>
                            </div>
                        </div>
                    </Card>
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                        <Card className="border-0 p-0">
                            <h3 className="mb-4 text-lg font-semibold text-content dark:text-content">
                                Parameter Matrix
                            </h3>
                            <Table>
                                <TableHead>
                        ...
                        <Card className="border-0 p-0">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <p className="text-sm text-content-subtle dark:text-content-placeholder">Total Nilai</p>
                                    <div className="flex items-center gap-3">
                                        <p className="text-2xl font-semibold text-content dark:text-content">
                                            {totalPercentage.toFixed(1)}%
                                        </p>
                                ...
                                {/* Ruler Visual */}
                                <div>
                                    <p className="text-sm text-content-subtle dark:text-content-placeholder mb-2">Score Ruler</p>
                ...
                <Card className="border-0 p-0">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="text-right">
                                <p className="text-sm text-content-subtle dark:text-content-placeholder">Quality Score</p>
                                <p className="text-2xl font-semibold text-content dark:text-content">
                                    {competencies.filter(c => c.isAchieved).length}/{competencies.length}
                                </p>
                            </div>
                    ...
                    {loading ? (
                        <div className="p-8 text-center text-sm text-content-subtle">Loading competencies...</div>
                        ...
                        <Table>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableHeaderCell className="text-center">Achieved</TableHeaderCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {competencies.map((comp, index) => (
                                    <TableRow key={comp.id}>
                                        ...
                                        <TableCell className="text-sm text-content-subtle dark:text-content-placeholder">
                                            {comp.description || "-"}
                                        </TableCell>
