...
import { deleteCompetency, getCompetencies } from "../../actions/competency-actions"
import { CompetencyForm } from "./CompetencyForm"

type Competency = Database["public"]["Tables"]["competency_library"]["Row"]

const availableRoles = Constants.public.Enums.project_role_enum.filter((role) => role !== "Admin")
...
export function WorkQualityTab() {
    ...
    useEffect(() => {
        const loadCompetencies = async () => {
            setLoading(true)
            try {
                const result = await getCompetencies(selectedRole === "All" ? undefined : selectedRole)
                if (result.success) {
    ...
    const handleDelete = async (id: string, name: string) => {
        if (!confirm(`Are you sure you want to delete the competency "${name}"?`)) return
        ...
        <div className="space-y-6">
            ...
            >
                {loading ? (
                    ...
                    <EmptyState
                        icon={<RiStarLine />}
                        title={selectedRole !== "All" ? `No competencies for ${selectedRole}` : "No competencies defined"}
                        description={
                            selectedRole !== "All"
            ...
            <div className="rounded-lg bg-primary/10 p-4">
                ...
                <p className="mt-2 text-sm text-primary/80">
                    Competencies defined here will automatically appear as checklists when admins grade employee projects.
                    The competencies shown are filtered by the employee&apos;s role in the project.
                </p>
                <p className="mt-2 text-sm text-primary/80">
                    <strong>Example:</strong> If an employee is assigned as &quot;UIX Designer&quot; on a project,
                    only competencies for &quot;UIX Designer&quot; role will appear in their Work Quality checklist.
                </p>
            </div>
