...
export function AttendanceStats({
    ...
    const activeSession = logs.find(l => l.date === todayStr && l.clock_out === null)
        ...
        <>
            ...
            <div className="grid gap-6 md:grid-cols-2 mb-8">
                ...
                <Card className="flex flex-col justify-between h-full">
                    <div>
                        ...
                        <dd className="mt-2 flex items-baseline gap-2">
                            <span className="text-2xl font-semibold text-content dark:text-content tabular-nums">
                                {currentTime ? format(currentTime, "HH:mm:ss") : "--:--:--"}
                            </span>
                            <span className="text-sm text-content-subtle dark:text-content-subtle">
                    ...
                    <div className="mt-6 flex flex-wrap gap-3 items-center">
                        {!activeSession ? (
                            <>
                                ...
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <Button size="sm" variant="secondary">
                                            ...
                                            <RiArrowDownSLine className="ml-2 size-4 text-content-subtle shrink-0" />
                                        </Button>
                                    </DropdownMenuTrigger>
                                ...
                                <Button
                                    size="sm"
                                    onClick={() => { setConfirmAction("CLOCK_IN"); setIsConfirmOpen(true) }}
                                    disabled={loading}
                                >
                            ...
                            <>
                                <Button
                                    size="sm"
                                    variant="destructive"
                                    onClick={() => { setConfirmAction("CLOCK_OUT"); setIsConfirmOpen(true) }}
                                    disabled={loading}
                                >
                ...
                <Card className="h-full">
                    <h3 className="text-sm font-medium text-content-subtle dark:text-content-subtle">Session Info</h3>
                    <dl className="mt-4 divide-y divide-zinc-200 dark:divide-zinc-800 text-sm">
                        <div className="py-2 flex justify-between items-center">
                            <dt className="text-content-subtle dark:text-content-subtle">First Clock In</dt>
                            <dd className="font-medium text-content dark:text-content">
                                {activeSession ? formatTime(activeSession.clock_in) : "--:--"}
                            </dd>
                        </div>
                        <div className="py-2 flex justify-between items-center">
                            <dt className="text-content-subtle dark:text-content-subtle">Working Duration</dt>
                            <dd className="font-medium text-content dark:text-content tabular-nums">
                                {activeSession && currentTime ? (() => {
                                    ...
                                    if (activeSession.is_break && activeSession.break_start) {
                                        const breakStart = new Date(activeSession.break_start).getTime()
                        ...
                        <div className="py-2 flex justify-between items-center">
                            <dt className="text-content-subtle dark:text-content-subtle">Activity</dt>
                            <dd className="text-content dark:text-content font-medium">
                                {activeSession ? (activeSession.is_break ? 'On Break' : 'Working') : '-'}
                            </dd>
                        </div>
                    </dl>
                </Card>
            ...
            <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
                <DialogContent className="sm:max-w-md">
                    ...
                    <DialogHeader>
                        <DialogTitle>
                            {confirmAction === "CLOCK_IN" ? "Confirm Clock In" : "Confirm Clock Out"}
                        </DialogTitle>
