...
export function QuarterFilter({
    ...
    const handleYearChange = (yearStr: string) => {
        ...
        const newValue = selectedQuarter === "All" ? `${year}-All` : `${year}-${selectedQuarter}`
        onChange?.(newValue)
    }

    const handleQuarterChange = (quarter: string) => {
        const newValue = quarter === "All" ? `${selectedYear}-All` : `${selectedYear}-${quarter}`
        onChange?.(newValue)
    }
    ...
    return (
        <div className={cx("flex items-center gap-3", className)}>
            ...
            {/* Quarter Buttons */}
            <div className="inline-flex items-center rounded-md bg-muted dark:bg-surface px-1 h-[34px]">
                    ...
                    <button
                        ...
                        className={cx(
                            ...
                            selectedQuarter === quarter.value
                                ? "bg-surface dark:bg-hover text-content dark:text-content"
                                : "text-content-subtle dark:text-content-placeholder hover:text-content dark:hover:text-content"
                        )}
                    >
                        {quarter.label}
                    </button>
