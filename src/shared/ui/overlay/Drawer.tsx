...
/**
 * Drawer component for side-anchored overlays.
      ...
      <DrawerOverlay>
        <DrawerPrimitives.Content
          ...
          className={cx(
            ...
            "fixed inset-y-2 z-[80] mx-auto flex w-[95vw] flex-1 flex-col overflow-y-auto rounded-md border border-neutral-primary p-4 shadow-lg focus:outline-none max-sm:inset-x-2 sm:inset-y-2 sm:right-2 sm:max-w-lg sm:p-0",
            // background color
            "bg-surface-neutral-primary dark:bg-surface-neutral-primary",
            // transition
...
>(({ children, className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className="border-border flex items-start justify-between gap-x-4 border-b pb-4"
      {...props}
    >
      ...
      <DrawerPrimitives.Close asChild>
        <Button
          variant="ghost"
          className="hover:bg-muted dark:hover:bg-muted aspect-square p-1"
        >
...
>(({ className, ...props }, forwardedRef) => {
  return (
    <DrawerPrimitives.Description
      ref={forwardedRef}
      className={cx("text-content-subtle dark:text-content-subtle", className)}
      {...props}
    />
  )
})
...
const DrawerFooter = ({
  ...
  return (
    <div
      className={cx(
        "dark:border-border flex flex-col-reverse border border-t pt-4 sm:flex-row sm:justify-end sm:space-x-2",
        className,
      )}
      {...props}
    />
