...
import { cx, percentageFormatter } from "@/shared/lib/utils"
  ...
  return (
    <li
      className={cx(
        ...
        hasOnValueChange
          ? "bg-transpaent hover:bg-muted dark:hover:bg-hover cursor-pointer"
          : "cursor-default",
      )}
      onClick={(e) => {
        e.stopPropagation()
  ...
  return (
    <button
      ...
      className={cx(
        ...
        disabled
          ? "text-content-placeholder dark:text-content-subtle cursor-not-allowed"
          : "text-content-subtle hover:bg-muted hover:text-content dark:text-content-subtle dark:hover:bg-hover dark:hover:text-content cursor-pointer",
      )}
      disabled={disabled}
      onClick={(e) => {
        e.stopPropagation()
  ...
  return (
    <div ref={legendRef} className="flex items-center justify-end">
      <Legend
        ...
        colors={
          filteredPayload.map(
            (entry) => categoryColors.get(String(entry.value)) ?? "gray",
      ...
      >
        <div className="flex items-start justify-between gap-2 border-b border-inherit p-2">
          ...
          {evolution !== undefined && (
            <Badge variant="zinc">
              {percentageFormatter(
                typeof evolution === "number" ? evolution : 0,
              )}
            </Badge>
          )}
        </div>
      ...
      <div ref={ref} className={cx("h-80 w-full", className)} {...other}>
        <ResponsiveContainer>
          ...
          >
            {showGridLines ? (
              <CartesianGrid
                className={cx("stroke-border dark:stroke-border stroke-1")}
                horizontal={true}
                vertical={false}
              />
            ) : null}
                ...
                activeDot={(props: any) => {
                  ...
                  return (
                    <Dot
                      ...
                      onClick={(_, event) =>
                        onDotClick(props as DotProps, event)
                      }
                    />
                  )
                }}
