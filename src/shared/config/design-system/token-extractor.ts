...
export function readGlobalsCss(): string {
    try {
        ...
        throw new Error('Failed to read globals.css file. Please ensure the file exists at src/app/globals.css');
    }
}
 ...
 */
function parseCssVariable(line: string): { name: string; value: string } | null {
...
export function extractTypographyTokens(css: string): TypographyToken[] {
    ...
    const fontSizes: Map<string, { fontSize: string; lineHeight?: string }> = new Map();
    ...
    for (const line of lines) {
        ...
        if (variable.name.startsWith('text-tremor-') && !variable.name.includes('line-height')) {
...
export function extractSpacingTokens(css: string): SpacingToken[] {
    ...
    for (const line of lines) {
        ...
        if (variable.name.startsWith('spacing-') || variable.name.startsWith('space-')) {
            tokens.push({
                name: variable.name,
                value: variable.value,
                cssVariable: `--${variable.name}`,
