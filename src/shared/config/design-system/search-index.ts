...
export function buildTokenIndex(): SearchableItem[] {
    ...
    // Color tokens
    const colorScales = ['primary', 'neutral', 'success', 'warning', 'danger', 'info']
    ...
    items.push({
        ...
        category: 'Typography',
        keywords: ['typography', 'font', 'text', 'size', 'weight', 'heading', 'body'],
        path: '#typography',
    })
 ...
 */
function calculateScore(item: SearchableItem, query: string, matchedFields: string[]): number {
    let score = 0
 ...
 */
function matchesQuery(item: SearchableItem, query: string): { matches: boolean; matchedFields: string[] } {
    const normalizedQuery = normalizeQuery(query)
 ...
 */
export function filterByType(results: SearchResult[], type: 'component' | 'token'): SearchResult[] {
 ...
 */
export function getTopResults(results: SearchResult[], limit: number): SearchResult[] {
