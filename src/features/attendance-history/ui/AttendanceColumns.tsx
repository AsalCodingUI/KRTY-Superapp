...
import { Badge, Button, DataTableColumnHeader } from "@/shared/ui";
...
export const createAttendanceColumns = (onRequestDelete?: (id: string) => void) => [
    columnHelper.accessor("clock_in", {
        header: ({ column }) => (
            <DataTableColumnHeader column={column} title="Clock In" className="whitespace-nowrap" />
        ),
        cell: ({ row }) => <span className="tabular-nums">{formatTime(row.getValue("clock_in"))}</span>,
        meta: { className: "w-1/5", displayName: "Clock In" },
    }),
    columnHelper.accessor("clock_out", {
        header: ({ column }) => (
            <DataTableColumnHeader column={column} title="Clock Out" className="whitespace-nowrap" />
        ),
        cell: ({ row }) => {
    ...
    columnHelper.display({
        id: "total_work",
        header: ({ column }) => (
            <DataTableColumnHeader column={column} title="Total Work" className="whitespace-nowrap" />
        ),
        cell: ({ row }) => <span className="tabular-nums">{calculateDuration(row.original)}</span>,
    ...
    columnHelper.accessor("status", {
        ...
        cell: ({ row }) => {
            ...
            return <Badge variant={status === 'Cuti Masuk' ? 'warning' : 'zinc'}>{status}</Badge>
        },
