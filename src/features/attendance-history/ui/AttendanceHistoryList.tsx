...
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger, Badge, DataTable, EmptyState } from "@/shared/ui"
...
export function AttendanceHistoryList({ logs, onRequestDelete }: AttendanceHistoryListProps) {
    ...
    const columns = useMemo(() => createAttendanceColumns(onRequestDelete), [onRequestDelete])
    ...
    return (
        <Accordion type="multiple" className="space-y-3">
            {groupedLogs.map(([date, dailyLogs]) => {
                ...
                const totalDailySeconds = dailyLogs.reduce((acc, log) => acc + getDurationSeconds(log), 0)
                        ...
                        <AccordionTrigger>
                            <div className="flex w-full items-center justify-between pr-4">
                                <div className="flex flex-col items-start sm:flex-row sm:items-center sm:gap-3">
                                    ...
                                    <span className="text-xs text-content-subtle dark:text-content-subtle font-normal">
                                ...
                                <div className="flex items-center gap-3">
                                    {isDayActive ? (
                                        ...
                                        <span className="text-sm font-medium text-content-subtle dark:text-content-placeholder tabular-nums">
