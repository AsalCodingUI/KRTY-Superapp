...
// Type for chart payload items from Recharts
type ChartPayload = {
  value: number | string
  name: string
  color: string
  ...
  return (
    <li
      className={cx(
        ...
        hasOnValueChange
          ? "bg-transpaent cursor-pointer hover:bg-muted dark:hover:bg-hover"
          : "cursor-default",
      )}
      onClick={(e) => {
        e.stopPropagation()
  ...
  return (
    <button
      ...
      className={cx(
        ...
        disabled
          ? "cursor-not-allowed text-content-placeholder dark:text-content-subtle"
          : "cursor-pointer text-content-subtle hover:bg-muted hover:text-content dark:text-content-subtle dark:hover:bg-hover dark:hover:text-content",
      )}
      disabled={disabled}
      onClick={(e) => {
        e.stopPropagation()
...
const ChartLegend = (
  ...
  return (
    <div ref={legendRef} className="flex items-center justify-end">
      <Legend
        categories={filteredPayload.map((entry) => String(entry.value))}
        colors={filteredPayload.map((entry) =>
          categoryColors.get(String(entry.value)) ?? "gray",
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-2 border-b border-inherit p-2">
          ...
          {evolution !== undefined && (
            <Badge variant="zinc">
              {percentageFormatter(typeof evolution === 'number' ? evolution : 0)}
            </Badge>
          )}
        </div>
        <div className={cx("space-y-1 p-2")}>
          {filteredPayload.map((payload, index: number) => {
            const payloadData = payload.payload
            return (
              <ChartTooltipRow
                key={`id-${index}`}
                value={valueFormatter(typeof payload.value === 'number' ? payload.value : 0)}
                name={String(
                  index === 0
                    ? (payloadData as Record<string, unknown>).formattedDate
                    : (payloadData as Record<string, unknown>).previousFormattedDate
                )}
      ...
      <div ref={ref} className={cx("h-80 w-full", className)} {...other}>
        <ResponsiveContainer>
          ...
          >
            {showGridLines ? (
              <CartesianGrid
                className={cx("stroke-border stroke-1 dark:stroke-border")}
                horizontal={true}
                vertical={false}
              />
            ) : null}
            ...
            <Tooltip
              ...
              cursor={{ stroke: "hsl(var(--border-default))", strokeWidth: 1 }}
              offset={20}
              position={{ y: 0 }}
              content={
                showTooltip ? (
              ...
              <Line
                ...
                activeDot={(props: any) => {
                  ...
                  return (
                    <Dot
                      className={cx(
                        "stroke-surface dark:stroke-surface",
                        onValueChange ? "cursor-pointer" : "",
                        getColorClassName(
                          categoryColors.get(
                            dataKey,
                          ) as AvailableChartColorsKeys ?? "gray",
                          "fill",
                        ),
                      )}
                      cx={cxCoord}
                      ...
                      fill=""
                      stroke={stroke}
                      strokeLinecap={strokeLinecap} // Cast if needed, but Dot accept string
                      strokeLinejoin={strokeLinejoin}
                      strokeWidth={strokeWidth}
                      onClick={(_, event) => onDotClick(props as DotProps, event)}
                    />
                  )
                }}
                dot={(props: any) => {
