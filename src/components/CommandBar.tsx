...
const shortcutStyles = cx(
  "hidden h-6 select-none items-center justify-center rounded-md bg-hover px-2 font-mono text-xs text-content-subtle ring-1 ring-inset ring-border transition sm:flex",
)
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cx(
        "px-3 py-2.5 text-sm tabular-nums text-content-subtle",
        className,
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cx(
        "relative flex items-center rounded-lg bg-surface px-1 shadow-md border border-border",
        className,
      )}
      {...props}
    />
...
const CommandBarCommand = React.forwardRef<HTMLButtonElement, CommandProps>(
  (
    ...
    return (
      <span
        className={cx(
          "flex items-center gap-x-2 rounded-lg bg-surface p-1 text-base font-medium text-content outline-none transition focus:z-10 sm:text-sm",
      ...
      >
        <button
          ...
          className={cx(
            // base
            "flex items-center gap-x-2 rounded-md px-1 py-1 hover:bg-hover",
            // focus
            "focus-visible:bg-hover focus-visible:hover:bg-hover",
