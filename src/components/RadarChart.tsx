...
const RadarChart = React.forwardRef<HTMLDivElement, RadarChartProps>(
    (
        ...
        return (
            <div ref={ref} className={cx("w-full h-80", className)} {...props}>
                <ResponsiveContainer width="100%" height="100%">
                    <RechartsRadarChart cx="50%" cy="50%" outerRadius="80%" data={data}>
                        ...
                        {categories.map((category, i) => {
                            ...
                            const colorClass = colorMap[colorKey] || "stroke-content-subtle fill-content-subtle"
                            ...
                            <Tooltip
                                ...
                                content={({ active, payload, label }) => {
                                    if (active && payload && payload.length) {
                                        return (
                                            <div className="rounded-md border-border bg-surface p-2 text-xs shadow-sm">
                                                <p className="mb-2 font-medium text-content dark:text-content">{label}</p>
                        ...
                        {showLegend && (
                            <Legend
                                ...
                                formatter={(value) => (
                                    <span className="text-sm text-content-subtle dark:text-content-placeholder ml-1">{value}</span>
                                )}
                            />
                        )}
                    </RechartsRadarChart>
